...
class Line(Base_Analysis): 
 ...
 # Calculates x_axis
    def x_axis(self, dataset_number, dataset_variable, sampling_rate=1):
        # Loading data 
...
# Filter Decay 
    def filter_decay(self, dataset_number, dataset_variable, sampling_rate=1): 
        variable = self.working_data[dataset_number][dataset_variable] 
        sub_variable = self.sub_sampling(variable, sampling_rate)
        ...
        for i in range(rows): 
            spe_dict[f'{i}'] = self.base_filter_decay(sub_variable[i])  
